...
count is = do
    ...
    f ref
    Map.toList <$> IORef.readIORef ref
  where
    f ref = Stream.read is >>= maybe
        (return ())
        (\a -> IORef.modifyIORef ref (Map.alter g $ l a) >> f ref)
    g Nothing  = Just 1
    g (Just n) = Just $ n + 1
    l = BC.map toLower
...
exam1 file parser = do
    r <- Stream.withFileAsInput file $ \is ->
        SA.parserToInputStream p is >>= count
